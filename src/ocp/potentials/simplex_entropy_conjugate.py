...
def _lambda_star_sorted(z: torch.Tensor, *, dim: int = -1, eps: float = 1e-12) -> torch.Tensor:
    ...
    idx = torch.arange(1, k + 1, device=z.device, dtype=z32.dtype).view(1, k)
    rhs = (B / (A + idx * B)) * prefix
    cond = u_sorted > rhs

    idx_int = torch.arange(1, k + 1, device=z.device).view(1, k)
    rho = (cond * idx_int).amax(dim=-1).clamp_min(1)
